...
from app.pipeline.summarization.generate_comprehensive_report import generate_comprehensive_report
...
async def get_filing_summary(
    ...
    report_type: str = Query("simple", description="Type of report: 'simple' for basic summary or 'comprehensive' for detailed analysis", regex="^(simple|comprehensive)$")
):
    """
    Provides a summary for a specified company filing.
    
    Two types of summaries are available:
    - 'simple': Basic summary with key takeaways (original format)
    - 'comprehensive': Detailed financial analysis report with revenue breakdown, margins, etc.
    
    The summary is generated based on pre-existing section summaries (Business, MD&A, Risk Factors).
    If a summary for the latest filing matching the criteria is already cached, it's returned.
    Otherwise, it's generated on-the-fly using GPT-4, stored, and then returned.
    """
    requested_form_type_upper = form_type.upper()
    print(f"Received request for {report_type} summary: Ticker={ticker}, Year={year}, Form Type={requested_form_type_upper}")

    if requested_form_type_upper != "10-K":
        raise HTTPException(
            status_code=400,
            detail=f"Currently, only '10-K' form types are fully supported for summaries. Requested: {requested_form_type_upper}"
        )
    ...
    try:
        if report_type == "comprehensive":
            print(f"Generating comprehensive report for {accession_number}")
            summary_text = await generate_comprehensive_summary_async_wrapper(accession_number)
        else:
            print(f"Generating simple summary for {accession_number}")
            summary_text = await generate_summary_async_wrapper(accession_number)
            
        if not summary_text:
            raise HTTPException(
                status_code=409, 
                detail=f"Could not generate {report_type} summary. Required section summaries may be missing."
            )
    ...
    except ValueError as ve: # Raised by generate functions if prerequisites missing
        ...
        # Consider more specific error codes based on exception types from generation functions
...
async def generate_summary_async_wrapper(accession_number: str):
    """Wrapper for simple summary generation."""
...
async def generate_comprehensive_summary_async_wrapper(accession_number: str):
    """Wrapper for comprehensive report generation."""
    try:
        # This call is synchronous and will block the event loop.
        # In production, this should be wrapped with run_in_executor
        comprehensive_report = generate_comprehensive_report(accession_number)
